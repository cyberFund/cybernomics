...
### Superagents
Superagent - self organized community of intelligent agents that solve particular task or set of tasks. Using metaphor we can divide any superagent into two types - nonliving or fully automated (artificial agents: contracts and nodes) and living that is semiautomated (biological agents or communities).
...
* Nodes - hardware agents that solve particular task. If some operation is more efficient to execute using specialised network of nodes we can reflect it on our model though we should understand that building specialized network of nodes is much more complicated than using ready to use infrastructure.
